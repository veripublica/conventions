...
3. States the convention version it targets — in its `--help`, its README, or
   its docs (e.g. *"conforms to veripublica conventions v0.6"*) — and that
   version is **tagged**.
...
**0.6.0** — while below `1.0.0`, any rule MAY change, and the stability boundary
...
The rule set is settled at `0.6.0` — twenty-seven proposals discussed and decided
...
parse the envelope by field name. `0.6.0` writes in the last accepted decision:
`reverted` joins the closed `outcome` set now that a tool emits it, and because a
closed set that grows is not an additive change, the stability key moves to
`0.6`. epublift adopts next; the spec leads, the tools follow, release by
release.
